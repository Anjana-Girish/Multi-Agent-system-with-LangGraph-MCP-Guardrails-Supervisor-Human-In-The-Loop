...
# aviationstack-mcp requires Python >=3.13, which may be newer than
# the interpreter running this app (e.g. a conda env pinned to an
# older Python). It runs as its own stdio subprocess, so it doesn't
# need to share that interpreter - it gets its own dedicated venv.
AVIATION_VENV_DIR = PROJECT_DIR / ".mcp_aviationstack_venv"
AVIATION_PYTHON = (
    AVIATION_VENV_DIR / "Scripts" / "python.exe"
    if os.name == "nt"
    else AVIATION_VENV_DIR / "bin" / "python"
)
...
client = MultiServerMCPClient(
    {
        "tavily": {
         ...
         "aviationstack": {
            ...
            # Dedicated venv (Python >=3.13) instead of sys.executable,
            # since the app's own interpreter may be older and
            # aviationstack-mcp requires >=3.13. Also avoids depending
            # on uvx being installed/allowed to run.
